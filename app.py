...
def daily_full_brief_html(
    ...
    return f"""
        <div class="ast-card" style="border:1px solid rgba(190,160,210,.50); background:rgba(255,255,255,.90);">
            <div class="ast-title" style="font-size:1.22rem;">🌙 오늘 브리핑 · 하루를 이렇게 쓰세요</div>
            <div class="small-note" style="font-size:0.95rem; line-height:1.86;">
                <strong>한눈에 보면</strong><br>
                {opening}<br><br>

                <strong>🕰️ 시간 흐름</strong><br>
                {morning}<br><br>
                {afternoon}<br><br>
                {evening}<br><br>

                <strong>💰 투자 브리핑</strong><br>
                {invest_head} {invest_action}<br>
                {invest_risk}<br><br>

                <strong>💌 연락·연애 브리핑</strong><br>
                {relation}<br><br>

                <strong>📚 공부·컨디션 브리핑</strong><br>
                {study_text} {condition_text}<br><br>

                <strong>✅ 오늘 해둘 것</strong><br>
                {"<br>".join(actions)}<br><br>

                <div style="padding:11px 12px; border-radius:12px; background:rgba(193,166,220,.11); border:1px solid rgba(193,166,220,.24);">
                    {one_line}
                </div>
            </div>
        </div>
