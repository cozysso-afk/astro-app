...
.lock-orb {
    ...
    place-items: center;
    font-size: 30px;
    background: radial-gradient(circle at 35% 30%, #ffffff 0%, #eadfff 34%, #b994e6 66%, #76549a 100%);
    box-shadow: 0 0 38px rgba(164, 125, 211, .32);
...
def _configured_app_pin():
    """Read the private PIN only from Streamlit Secrets; never from public source code."""
    return _secret_text("APP_PIN", "")


def require_app_unlock():
    """Stop before any personal chart/forecast UI is rendered unless PIN is verified."""
    ...
    st.markdown(
        ...
        <div class="lock-wrap">
            ...
            <div class="lock-title">별빛의 운명</div>
            <div class="lock-sub">Private Astrology Room<br>개인 운세 데이터 보호를 위해 PIN을 입력해 주세요.</div>
        </div>
        """,
    ...
    if not configured_pin:
        st.warning("🔐 Streamlit Secrets에 APP_PIN이 아직 설정되지 않았습니다.")
        st.code('APP_PIN = "6자리 이상 PIN"', language="toml")
        st.caption("Streamlit Cloud → Manage app → Settings → Secrets에 저장한 뒤 앱을 재부팅해 주세요.")
        st.stop()

    if len(configured_pin) < 6:
        st.error("보안을 위해 APP_PIN은 6자리 이상으로 설정해 주세요.")
        st.stop()
    ...
    if now < blocked_until:
        ...
        st.error(f"PIN 입력을 여러 번 틀렸습니다. 약 {remaining}초 후 다시 시도해 주세요.")
        st.stop()
    ...
    if submitted:
        if hmac.compare_digest(entered_pin.strip(), configured_pin):
            ...
            if failures >= 5:
                ...
                st.error(f"PIN이 맞지 않습니다. 잠시 잠기기까지 {5 - failures}회 남았습니다.")

    st.markdown(
        '<div class="lock-note">PIN과 기본 프로필은 공개 코드가 아니라 Streamlit Secrets에서 읽도록 구성했습니다. '
        '인증은 현재 Streamlit 세션 동안 유지됩니다.</div>',
        unsafe_allow_html=True,
    )
    st.stop()
...
# 사용 중 즉시 다시 잠글 수 있는 수동 잠금 버튼
with st.sidebar:
    if st.button("🔒 앱 다시 잠그기", key="astro_manual_lock", use_container_width=True):
...
RETURN_CONFIG = {
    ...
    "Mars": {"window_days": 800, "step_hours": 12.0},
}
...
def scan_intraday(day_value, start_time, end_time, step_minutes, natal_lons, natal_houses):
    ...
    Sun/Moon/Mercury/Venus/Mars are recalculated at every slot; only slow planets use the midpoint.
    """
...
st.title("🌙 별빛의 운명")
st.caption(f"{EPHEMERIS_USED} 기반 · Tropical · Whole Sign 주 기준 · Placidus 보조")

# 공개 GitHub 소스에 개인 생년월일/출생시각을 남기지 않도록 기본값도 Secrets에서 읽는다.
def _profile_date_default():
    raw = _secret_text("PROFILE_BIRTH_DATE", "2000-01-01")
    try:
        return date.fromisoformat(raw)
    except ValueError:
        st.warning("PROFILE_BIRTH_DATE 형식은 YYYY-MM-DD여야 합니다. 임시 기본값을 사용합니다.")
        return date(2000, 1, 1)


def _profile_time_default():
    raw = _secret_text("PROFILE_BIRTH_TIME", "12:00")
    try:
        return dt_time.fromisoformat(raw)
    except ValueError:
        st.warning("PROFILE_BIRTH_TIME 형식은 HH:MM이어야 합니다. 임시 기본값을 사용합니다.")
        return dt_time(12, 0)
...
PROFILE_BIRTH_DATE_DEFAULT = _profile_date_default()
PROFILE_BIRTH_TIME_DEFAULT = _profile_time_default()
PROFILE_BIRTH_PLACE_DEFAULT = _profile_place_default()
...
with st.expander("👤 출생정보 · 조회날짜 입력/수정", expanded=True):
    ...
    birth_place = st.selectbox(
        "출생 지역",
        place_options,
        index=place_options.index(PROFILE_BIRTH_PLACE_DEFAULT),
...
with tabs[6]:
    ...
    status_items = [
        ...
        "Intraday: Sun/Moon/Mercury/Venus/Mars recalculated each slot",
    ]
