...
def daily_full_brief_html(market_rows, life_rows, realize_windows, entry_windows, risk_windows, study_windows, contact_windows):
    """예약 브리핑처럼 숫자를 '오늘 어떻게 써야 하는지'로 번역한 하루 총평."""
    day = {
    ...
    ordered = sorted(positive, key=lambda k: day[k], reverse=True)
    lead, second, low = ordered[0], ordered[1], ordered[-1]
    gap = day[lead] - day[second]

    if day[lead] >= 68 and gap >= 6:
        overall = (
            f"오늘은 <strong>{PERIOD_LABELS[lead]}</strong> 쪽이 하루 전체에서 가장 또렷합니다. "
            f"하루 평균 {day[lead]}/100으로 다른 분야보다 한 단계 앞서므로, 중요한 선택을 전부 벌이기보다 "
            f"이 분야에 힘을 집중하는 편이 효율적입니다."
        )
    elif day[lead] >= 55:
        overall = (
            f"오늘은 <strong>{PERIOD_LABELS[lead]}</strong>과 {PERIOD_LABELS[second]} 흐름이 상대적으로 살아 있습니다. "
            f"강하게 밀어붙이는 날이라기보다, 좋은 시간대를 골라 한두 가지를 정확히 처리할수록 체감이 좋아지는 날입니다."
        )
    else:
        overall = (
            f"오늘은 한 분야가 압도적으로 치고 나가기보다는 전체적으로 잔잔한 편입니다. "
            f"그중에서는 <strong>{PERIOD_LABELS[lead]}</strong>이 가장 낫고, {PERIOD_LABELS[low]}은 힘이 덜 실립니다. "
            "성과를 억지로 만들기보다 이미 정해둔 루틴과 기준을 지키는 쪽이 유리합니다."
        )
    ...
    if realize >= entry + 6:
        invest = (
            f"대주주님, 투자 쪽은 <strong>신규진입보다 수익실현·보유관리</strong>가 상대적으로 낫습니다 "
            f"(하루 평균 수익실현 {realize}, 신규진입 {entry}). "
            + (f"특히 <strong>{realize_time}</strong>가 가장 우호적인 후보 구간이라, 수익권 포지션이 있다면 목표가·저항대와 함께 분할 확정을 검토해보세요. " if realize_time else "")
        )
    elif entry >= realize + 6:
        invest = (
            f"대주주님, 오늘은 <strong>기존 포지션 정리보다 신규진입 탐색</strong>이 상대적으로 조금 더 낫습니다 "
            f"(하루 평균 신규진입 {entry}, 수익실현 {realize}). "
            + (f"후보 시간은 <strong>{entry_time}</strong>이지만, 점수만 보고 추격하지 말고 지지·수급 확인 뒤 작은 비중으로 시작하는 쪽이 안전합니다. " if entry_time else "")
        )
    else:
        invest = (
            f"대주주님, 투자에서는 수익실현 {realize}와 신규진입 {entry}의 차이가 크지 않아 <strong>방향성이 강한 날은 아닙니다.</strong> "
            "새 포지션을 억지로 만들기보다 보유 종목의 가격 조건이 맞을 때만 대응하는 편이 낫습니다. "
        )

    if risk >= 68:
        invest += (
            f"과열주의 평균이 <strong>{risk}/100</strong>으로 높은 편입니다. "
            + (f"특히 <strong>{risk_time}</strong> 전후에는 " if risk_time else "")
            + "‘놓치면 안 된다’는 마음으로 주문 기준을 바꾸지 말고, 수량·손절가를 먼저 고정해두세요."
        )
    elif risk >= 55:
        invest += (
            f"과열주의는 {risk}/100으로 중간 이상입니다. "
            "기회가 보여도 첫 주문 크기를 줄이고, 계획에 없던 추가매수만 피하면 훨씬 안정적입니다."
        )
    else:
        invest += "과열 신호는 상대적으로 낮지만, 실제 매매 판단은 가격·수급·거래량이 항상 우선입니다."
    ...
    if contact >= love + 5:
        relation = (
            f"관계운은 <strong>감정 결론보다 연락·접점 만들기</strong> 쪽이 더 낫습니다 "
            f"(연락 {contact}, 연애 {love}). "
            + (f"연락을 한다면 <strong>{contact_time}</strong> 전후가 상대적으로 자연스럽고, " if contact_time else "")
            + "길게 설명하거나 답을 재촉하기보다 가볍고 구체적인 한마디가 더 잘 맞습니다."
        )
    elif love >= contact + 5:
        relation = (
            f"관계운은 단순 연락 횟수보다 <strong>상대의 반응과 감정 온도</strong>를 보는 쪽이 더 중요합니다 "
            f"(연애 {love}, 연락 {contact}). 대화를 하게 된다면 관계를 정의하려 하기보다 실제 분위기와 반응을 관찰해보세요."
        )
    else:
        relation = (
            f"연락 {contact}, 연애 {love}가 비슷해서 관계 쪽은 큰 결론보다 <strong>작은 반응 확인</strong>이 더 어울립니다. "
            "먼저 움직이더라도 한 번에 답을 얻으려 하지 말고, 상대의 말투·속도·후속 반응을 보는 편이 좋습니다."
        )
    ...
    if study >= 60:
        study_text = (
            f"공부·시험은 하루 평균 <strong>{study}/100</strong>으로 쓸 만합니다. "
            + (f"특히 <strong>{study_time}</strong> 전후에는 " if study_time else "")
            + "새 진도보다 문제풀이·암기 회수·오답 정리처럼 결과가 바로 보이는 작업을 배치해보세요."
        )
    elif study >= 45:
        study_text = (
            f"공부·시험은 {study}/100으로 보통 흐름입니다. 오래 버티기보다 30~50분 단위로 끊어서 집중하고, "
            "오늘 반드시 끝낼 범위를 좁히는 편이 효율적입니다."
        )
    else:
        study_text = (
            f"공부·시험은 {study}/100으로 강한 날은 아닙니다. 어려운 새 범위를 넓히기보다 복습·정리·암기 유지처럼 "
            "실수를 줄이는 공부에 초점을 맞추세요."
        )

    if condition >= 60:
        study_text += f" 컨디션은 {condition}/100으로 비교적 받쳐주니 중요한 일은 미루지 않는 편이 좋습니다."
    elif condition < 42:
        study_text += f" 다만 컨디션이 {condition}/100으로 낮아, 일정 사이에 회복 시간을 일부러 넣는 게 필요합니다."
    else:
        study_text += f" 컨디션은 {condition}/100으로 무난하니 무리한 몰아치기만 피하면 됩니다."

    if risk >= 68:
        one_line = f"오늘의 한 줄: <strong>{PERIOD_LABELS[lead]}에는 힘을 쓰되, 투자 판단만큼은 속도보다 기준을 지키는 날.</strong>"
    elif day[lead] >= 60:
        one_line = f"오늘의 한 줄: <strong>{PERIOD_LABELS[lead]}에 우선순위를 두고, 좋은 시간대에 중요한 한 가지를 끝내는 날.</strong>"
    else:
        one_line = "오늘의 한 줄: <strong>크게 벌이기보다 루틴을 지키고, 반응이 오는 분야만 선택적으로 움직이는 날.</strong>"

    return f"""
        <div class="ast-card" style="border:1px solid rgba(190,160,210,.48); background:rgba(255,255,255,.86);">
            <div class="ast-title">🌙 하루 총평 · 오늘 이렇게 보세요</div>
            <div class="small-note" style="font-size:0.92rem; line-height:1.82;">
                <strong>전체 흐름</strong><br>{overall}<br><br>
                <strong>💰 투자</strong><br>{invest}<br><br>
                <strong>💌 연락·연애</strong><br>{relation}<br><br>
                <strong>📚 공부·컨디션</strong><br>{study_text}<br><br>
                {one_line}
            </div>
        </div>
    """
