...
def weekly_overview_text(rows):
    ...
    parts = [
        f"이번 7일은 앱 계산상 **{PERIOD_LABELS[lead_key]}** 흐름이 상대적으로 가장 선명해. "
        f"평균 {avgs[lead_key]}/100이고, 특히 **{lead_day['label']}({lead_day[lead_key]}/100)**가 피크야."
    ]
    if risk_day:
        if risk_day["투자주의"] >= 70:
            parts.append(
                f"투자 쪽은 **{risk_day['label']}**에 과열주의가 {risk_day['투자주의']}/100까지 올라가서, "
                "그날은 좋은 점수가 같이 보여도 추격·몰빵보다 주문 전 재확인이 우선이야."
            )
        elif risk_avg >= 55:
            parts.append("투자심리는 주중에 다소 흔들릴 수 있어서 진입보다 리스크 관리 비중을 조금 높이는 편이 좋아.")
        else:
            parts.append("투자 과열 신호는 주간 평균 기준으로 아주 강한 편은 아니지만, 실제 매매 기준이 항상 우선이야.")
    return " ".join(parts)
...
def weekly_topic_text(rows, key, risk=False):
    ...
    if risk:
        if best[key] >= 70:
            core = f"{label}는 평균 {avg}/100. 가장 경계할 날은 **{best['label']} {best[key]}/100**이야."
        else:
            core = f"{label}는 평균 {avg}/100으로 극단적인 경고 신호는 상대적으로 적어. 최고치는 **{best['label']} {best[key]}/100**."
        return f"{core} 흐름은 {trend}."

    if avg >= 68:
        tone = "주간 전체적으로 힘이 실리는 편"
    elif avg >= 55:
        tone = "쓸 만한 구간이 분명히 있는 편"
    elif avg >= 40:
        tone = "날짜 선택이 중요한 보통 흐름"
    else:
        tone = "전체적으로 강한 신호는 적은 편"
    return (
        f"{label}는 평균 **{avg}/100**으로 {tone}. "
        f"최고는 **{best['label']} {best[key]}/100**, 가장 낮은 날은 **{worst['label']} {worst[key]}/100**이고, {trend}."
    )


def daily_period_one_liner(row):
    candidates = ["수익실현", "신규진입", "학업", "연락", "연애", "컨디션"]
    lead = max(candidates, key=lambda k: row.get(k, 0))
    second = sorted(candidates, key=lambda k: row.get(k, 0), reverse=True)[1]
    risk = row.get("투자주의", 0)
    text = (
        f"**{row['label']}** · {PERIOD_LABELS[lead]} {row[lead]}이 가장 두드러지고, "
        f"{PERIOD_LABELS[second]} {row[second]}가 뒤를 받쳐."
    )
    if risk >= 70:
        text += f" ⚠️ 투자 과열주의 {risk}로 매매는 특히 보수적으로."
    elif risk >= 58:
        text += f" 투자 과열주의 {risk}라 주문 전 한 번 더 체크."
    return text
...
def segment_summary(rows, label):
    ...
    caution = ""
    if risk >= 68:
        caution = f" · 투자과열 평균 {risk}로 보수적 대응 필요"
    elif risk >= 55:
        caution = f" · 투자과열 평균 {risk}, 주문 전 재확인 권장"
    return (
        f"**{label} ({start}~{end})** · 중심 테마는 **{PERIOD_LABELS[lead]} {avgs[lead]}**. "
        f"{period_trend(rows, lead)}{caution}"
    )
...
def monthly_overview_text(rows):
    ...
    return (
        f"이번 달은 숫자 하나보다 **월초→월중→월말의 이동**을 보는 게 중요해. "
        f"전체 평균에서는 **{PERIOD_LABELS[lead]} {avgs[lead]}/100**이 가장 강하고, "
        f"그 테마의 최고점은 **{best['label']} {best[lead]}/100**이야. "
        + (f"투자 과열 신호가 가장 높은 날은 **{risk['label']} {risk['투자주의']}/100**이니 그날은 특히 리스크 기준을 먼저 봐." if risk else "")
    )
            ...
            for row, window in weekly_detail.get(key, []):
                st.markdown(
                    f'''<div class="window-card {css}">
                        ...
                        <span class="small-note">{action_advice(score_key, window["score"])}</span>
                    </div>''',
                    unsafe_allow_html=True,
                )
...
with tabs[2]:
    ...
    if month_rows:
        ...
        for title, key, is_risk in monthly_interpretations:
            ...
            if is_risk:
                body = (
                    f"월간 평균 **{avg}/100**. 가장 경계할 날짜는 **{best['label']} {best[key]}/100**. "
                    f"{period_trend(month_rows, key)}. 높은 날에는 좋은 신호가 같이 있어도 주문 규칙을 우선해."
                )
            else:
                body = (
                    f"월간 평균 **{avg}/100** · {period_trend(month_rows, key)}. "
                    f"가장 힘이 실리는 날은 **{best['label']} {best[key]}/100**, "
                    f"가장 잔잔한 날은 **{worst['label']} {worst[key]}/100**이야."
                )
            st.markdown(
                f"""
                <div class="ast-card" style="padding:15px 17px;">
