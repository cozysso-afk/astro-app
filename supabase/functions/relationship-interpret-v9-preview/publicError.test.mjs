...
const reunionProse=tag=>`${tag}에서 계산 근거가 뜻하는 바와 현실에서 확인할 장면을 구분해서 설명한다. ${tag} 단계가 다음 단계와 어떻게 다른지도 함께 설명해 과장 없이 읽을 수 있게 한다. `.repeat(12);
const reunionEvidenceIds=['R2:test:1','R2:test:2','R2:test:3','R2:test:4','R2:test:5','R2:test:6'];
const reunionEvidence={buildReunionEvidenceV2:()=>({version:'reunion-evidence-v2.0',policy:'test',coverage:{},questions:{why_reconnect:{evidence_refs:reunionEvidenceIds.slice(0,2),support_refs:reunionEvidenceIds.slice(0,2),counter_refs:[],families:['a','b']},initiative:{evidence_refs:reunionEvidenceIds.slice(1,3),support_refs:reunionEvidenceIds.slice(1,3),counter_refs:[],families:['a','b']},timing:{evidence_refs:reunionEvidenceIds.slice(2,4),support_refs:reunionEvidenceIds.slice(2,4),counter_refs:[],families:['a','b']},rebuild:{evidence_refs:reunionEvidenceIds.slice(3,5),support_refs:reunionEvidenceIds.slice(3,5),counter_refs:[],families:['a','b']},repeat_risks:{evidence_refs:reunionEvidenceIds.slice(4,6),support_refs:[],counter_refs:reunionEvidenceIds.slice(4,6),families:['a','b']}},evidence:reunionEvidenceIds.map(id=>({id})),convergence:[]})};
const fixture={headline:'합성 해설',overview:prose,chemistry:prose,emotional_dynamic:prose,communication:prose,conflict_pattern:prose,power_boundaries:prose,long_term:prose,timing:prose,reunion_context:prose,felt_scenarios:[prose,prose,prose],practical_advice:[prose],top_aspects:[{label:'근거',meaning:prose}],limits:prose,reunion_reading:{bottom_line:prose,incoming_contact:prose},reunion_synthesis_v2:{summary:reunionProse('재회 전체 요약'),why_reconnect:{conclusion:reunionProse('다시 신경 쓰이는 이유 결론'),interpretation:reunionProse('다시 신경 쓰이는 이유 해설'),evidence_refs:reunionEvidenceIds.slice(0,2)},initiative:{conclusion:reunionProse('선연락 주체 결론'),interpretation:reunionProse('선연락 주체 해설'),evidence_refs:reunionEvidenceIds.slice(1,3)},timing:{conclusion:reunionProse('현재 단계와 다음 단계'),windows:[{period:'2027-01',meaning:reunionProse('후보 시기 의미'),evidence_refs:reunionEvidenceIds.slice(2,4)}],evidence_refs:reunionEvidenceIds.slice(2,4)},rebuild:{conclusion:reunionProse('관계 재구축 조건'),conditions:[reunionProse('재구축 확인 장면')],evidence_refs:reunionEvidenceIds.slice(3,5)},repeat_risks:{conclusion:reunionProse('반복 위험 결론'),patterns:[reunionProse('반복 위험 장면')],evidence_refs:reunionEvidenceIds.slice(4,6)},convergence:[],precision_note:reunionProse('정밀도 안내')},marriage_reading:Object.fromEntries(['bottom_line','bond','emotional_home','daily_life','intimacy_resources','conflict_repair','commitment_or_current_cycle','timing','caution','precision_note'].map(k=>[k,prose]))};
...
const calculationFor=purpose=>purpose==='reunion'?{result:{reunion_hierarchy:{version:'test-hierarchy',as_of_date:'2026-09-19',validation:{status:'PASS'},weights:{},thresholds:{},stages:{},top_periods:[],nearest_window:null,initiative:{available:false,verdict:'판정 불가'},coverage:{western:true,saju:false,ziwei:false},limitations:[],score_meaning:'test',stability_structure:{}}}}:{result:{}};
