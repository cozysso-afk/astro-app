...
const DEFAULT_MODEL="gemini-3.7-flash",FALLBACK_MODEL="gemini-3.6-flash";
const BASE_VERSION="relationship-v11.6-reunion-compact-evidence",REUNION_VERSION="relationship-v11.7-reunion-direct-budget";
...
function versionFor(purpose:Purpose){return purpose==="reunion"?REUNION_VERSION:BASE_VERSION;}
function outputCaps(purpose:Purpose){return purpose==="reunion"?{first:9000,retry:6500}:{first:16000,retry:12000};}
...
[재회 reunion]
가장 먼저 사용자가 실제로 묻는 네 가지를 답한다. ① 상대 → 나 방향이 강한지/보통인지/약한지와 그 근거, ② 나 → 상대 방향과 그 근거, ③ 둘을 비교했을 때 어느 방향이 상대적으로 더 강한지, ④ 선택기간 안의 구체적인 재접점 시기 2~4개와 각 시기의 근거. 점수를 사건 확률이나 퍼센트로 바꾸지 말고 '선택기간 내 상대적 활성도'라고 명시한다. 그 다음 contact_recontact(연락·재접촉), emotional_reactivation(감정·관계 재활성), relationship_rebuilding(관계 재구축)을 분리해 연락이 오는 것과 실제 관계 회복이 이어지는 조건이 왜 다른지 설명한다. Secondary Progression을 Daily Transit보다 상위 시기근거로 두고, 날짜가 없으면 만들지 않는다. '대화를 해봐', '상대 반응을 봐' 같은 일반론만 반복하지 말고 실제 계산된 방향·날짜·애스펙트가 문단의 주어가 되게 쓴다.
...
function grounded(data:any,payload:any,p:Purpose,relaxed=false){
 ...
 if(p==="reunion"){
   const r=data.reunion_reading??{};
   if(String(r.bottom_line??"").length<need(160))return false;
   for(const k of ["incoming_contact","outgoing_contact","relationship_filter"])
     if(String(r[k]??"").length<need(100))return false;
   if(String(r.reconnection_windows??"").length<need(140))return false;
 }
 return true;
}

function modeInstruction(purpose:Purpose){return purpose==="compatibility"?"일반 연애 궁합이다. 표준 궁합 포인트와 사주 관계층을 빠짐없이 읽고 각 섹션을 충분히 길게 써라.":purpose==="reunion"?"재회운이다. 첫 문단부터 상대→나, 나→상대, 어느 방향이 상대적으로 더 강한지, 구체적 재접점 시기 2~4개를 답해라. 그 뒤 연락·재접촉 / 감정·관계 재활성 / 관계 재구축 지원층을 각각 따로 결론내라. 각 축의 수신·발신·동시 재접점 방향을 섞지 말고, 점수를 사건확률로 바꾸지 마라. Secondary Progression을 Daily Transit보다 상위 시기근거로 두고 하나의 재회 점수는 만들지 마라.":purpose==="marriage_unmarried"?"특정 상대가 있는 미혼 결혼궁합이다. 두 사람이 결혼생활로 들어갈 경우의 결속·정서적 집·생활 역할·돈/공유자원·친밀감·갈등회복·책임을 분리해 읽고, 결혼으로 공식화될 가능성과 프러포즈·약혼·결혼 결정이 강해지는 시기 흐름도 계산 근거 범위에서 적극적으로 제시하되 확정 사실처럼 단정하지 마라.":"이미 결혼한 두 사람의 결혼생활 분석이다. 결혼 가능성 표현은 금지하고 현재 결속·정서적 거리·생활 역할·공유재정/친밀감·반복갈등·회복 주기를 읽어라.";}
...
function relationshipEstimatedJobKrw(inputTokens:number,purpose:Purpose){const intro=Date.now()<=GEMINI_INTRO_END,inputRate=intro ? .75 : 1.5,outputRate=intro ? 3.75 : 7.5,thoughtReserve=3000,caps=outputCaps(purpose);const one=(output:number)=>((inputTokens/1_000_000)*inputRate+((output+thoughtReserve)/1_000_000)*outputRate)*GEMINI_USD_KRW_ESTIMATE;return one(caps.first)+one(caps.retry);}
...
async function generate(payload:any,purpose:Purpose,model:string,key:string,compactMode=false){const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),115000);try{const prompt=promptText(payload,purpose,compactMode),caps=outputCaps(purpose);const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`,{method:"POST",signal:controller.signal,headers:{"Content-Type":"application/json","x-goog-api-key":key},body:JSON.stringify({systemInstruction:{parts:[{text:SYSTEM}]},contents:[{role:"user",parts:[{text:prompt}]}],generationConfig:{responseMimeType:"application/json",responseSchema:schemaFor(purpose),maxOutputTokens:compactMode?caps.retry:caps.first,temperature:.32,thinkingConfig:{thinkingLevel:"medium"}}})});const rawText=await r.text();if(!r.ok)return publicRelationshipError("REL_GENERATION_FAILED",{model,usage:{prompt_tokens:0,candidate_tokens:0,thought_tokens:0,total_tokens:0}});let raw:any;try{raw=JSON.parse(rawText)}catch{return publicRelationshipError("REL_RESULT_INVALID",{model})}const u=usage(raw),parts=raw?.candidates?.[0]?.content?.parts??[];let txt=parts.filter((x:any)=>!x?.thought).map((x:any)=>x?.text??"").join("").trim()||parts.map((x:any)=>x?.text??"").join("").trim();txt=txt.replace(/^```(?:json)?\s*/i,"").replace(/\s*```$/i,"");try{const data=validate(JSON.parse(txt),purpose);if(!data||!grounded(data,payload,purpose,compactMode))return publicRelationshipError("REL_RESULT_INVALID",{model,usage:u});return {ok:true,data,model,interpreter_version:versionFor(purpose),usage:u};}catch{return publicRelationshipError("REL_RESULT_INVALID",{model,usage:u});}}catch(e){return publicRelationshipError(e instanceof DOMException&&e.name==="AbortError"?"REL_GENERATION_TIMEOUT":"REL_GENERATION_FAILED",{model,usage:{prompt_tokens:0,candidate_tokens:0,thought_tokens:0,total_tokens:0}});}finally{clearTimeout(timer);}}
async function calculate(payload:any,purpose:Purpose,preferred:string,key:string){let calls=0;const first:any=await generate(payload,purpose,preferred,key,false);calls+=1;const firstUsage=first.usage??{};if(first.ok)return {...first,usage:{...firstUsage,attempt_count:calls}};if(calls>=MAX_GEMINI_CALLS)return {...first,usage:{...firstUsage,attempt_count:calls}};const retryModel=preferred===DEFAULT_MODEL?FALLBACK_MODEL:preferred;const second:any=await generate(payload,purpose,retryModel,key,true);calls+=1;const total=addUsage(firstUsage,second.usage??{});if(second.ok)return {...second,usage:{...total,attempt_count:calls},...(retryModel!==preferred?{fallback_from:preferred}:{})};return publicRelationshipError(second.error_code??"REL_GENERATION_FAILED",{model:preferred,interpreter_version:versionFor(purpose),usage:{...total,attempt_count:calls}});}
