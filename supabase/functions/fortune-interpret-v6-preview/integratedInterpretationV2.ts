...
function strings(v:any,out:string[]=[]):string[]{if(typeof v==="string")out.push(v);else if(Array.isArray(v))for(const x of v)strings(x,out);else if(v&&typeof v==="object")for(const x of Object.values(v))strings(x,out);return out;}
...
export function inspectInterpretationQuality(data:any,payload:any){
  ...
  for(const row of map.values() as any){for(const v of [row?.date,row?.start,row?.end]){const d=isoDate(v);if(d)knownDates.add(d);}}
  const pStart=isoDate(payload?.period?.start),pEnd=isoDate(payload?.period?.end);if(pStart)knownDates.add(pStart);if(pEnd)knownDates.add(pEnd);
  for(const w of data?.key_windows??[]){const d=isoDate(w?.start);if(d&&!withinPeriod(d,payload))s2.push(`기간 밖 key_window 시작일: ${d}`);if(!Array.isArray(w?.evidence_refs)||!w.evidence_refs.length)s2.push(`근거 없는 key_window: ${w?.label||d}`);}
  for(const d of uniq(strings(data).flatMap(datesInText)))if(!knownDates.has(d))s2.push(`계산근거에서 찾을 수 없는 날짜 언급: ${d}`);
  stages.push(qualityStage(2,"근거 추적성",uniq(s2).slice(0,30)));

  const s3:string[]=[];
  const prose=strings(data).join("\n");
  const forbidden:[[RegExp,string]]|any=[
    [/\b\d+(?:\.\d+)?\s*%/g,"확률처럼 보이는 % 수치"],[/사건\s*확률|연락\s*확률|재회\s*확률|합격\s*확률/g,"사건 확률 표현"],[/무조건|100%|확실히\s*(?:연락|재회|합격|오른|내린)|반드시\s*(?:연락|재회|합격|수익)/g,"결정론적 미래 단정"],
  ];
  for(const [re,label] of forbidden){if(re.test(prose))s3.push(label);re.lastIndex=0;}
  for(const w of data?.key_windows??[]){const rows=(w?.evidence_refs??[]).map((r:string)=>map.get(r)).filter(Boolean) as any[];const pos=rows.some(r=>r.direction==="supportive"),neg=rows.some(r=>r.direction==="caution");if(w.signal==="활용"&&neg&&!pos)s3.push(`주의 근거만 있는데 활용으로 표시: ${w.label}`);if(w.signal==="주의"&&pos&&!neg)s3.push(`활용 근거만 있는데 주의로 표시: ${w.label}`);if(pos&&neg&&w.signal!=="혼합")s3.push(`상반 근거가 함께 있는데 혼합 아님: ${w.label}`);}
  ...
  for(const [topic,x] of Object.entries(data?.topic_analysis??{}) as any[]){if(x?.confidence==="높음"){const rows=(x?.evidence_refs??[]).map((r:string)=>map.get(r)).filter(Boolean) as any[];if(rows.length<2)s4.push(`${topic} 확신도 높음인데 근거 2개 미만`);if(!rows.some(r=>r.system==="western"))s4.push(`${topic} 확신도 높음인데 Western 근거 없음`);}}
  ...
  const minSummary=kind==="annual"?240:kind==="month"?170:110;if(String(data?.overall?.summary??"").length<minSummary)s5.push(`총평 깊이 부족(${String(data?.overall?.summary??"").length}/${minSummary})`);
  for(const w of data?.key_windows??[]){if(String(w?.summary??"").length<45)s5.push(`핵심 시기 설명이 너무 짧음: ${w?.label}`);if(String(w?.action??"").length<18)s5.push(`핵심 시기 행동이 너무 짧음: ${w?.label}`);if((w?.evidence_refs?.length??0)<(kind==="annual"?2:1))s5.push(`핵심 시기 근거 수 부족: ${w?.label}`);}
  for(const [topic,x] of Object.entries(data?.topic_analysis??{}) as any[]){const minReason=kind==="annual"?70:45;if(String(x?.reason??"").length<minReason)s5.push(`${topic} 근거 설명이 얕음`);if(!(x?.evidence_refs?.length))s5.push(`${topic} 근거 ID 없음`);}
  const generic=/좋은\s*기운|긍정적으로|마음을\s*열|천천히\s*해보|자신을\s*믿|잘\s*해낼/g;if((prose.match(generic)??[]).length>=3)s5.push("일반론 조언 반복이 많음");
  stages.push(qualityStage(5,"깊이·실용성",uniq(s5).slice(0,40)));
...
export function strictQualityRetryInstruction(report:any){
  ...
  return `\n\nQUALITY_RETRY: 이전 응답이 5단계 품질검증을 통과하지 못했다. 아래 실패를 모두 고쳐라. 근거 ID는 evidence_ledger에 실제 존재하는 값만 쓰고, 날짜를 새로 만들지 마라.\n${failed}`;
}
