...
- observation.transit은 현재 운행 행성, target은 출생차트의 비교 대상이다. 둘을 구분해 왜 개인에게 해당하는 근거인지 풀어쓴다. 제공되지 않은 출생 별자리·하우스·성향은 만들지 않는다.
- observed_transit_context의 날짜는 표본 관측일이다. 첫 관측을 시작일, 마지막 관측을 종료일로 쓰지 않는다. motion의 적용/분리는 접촉에 가까워짐/멀어짐이며 긍정/부정이나 상대의 접근/이탈이 아니다. 정확(Exact)은 엔진 허용범위 안이며 사건 발생 시각이 아니다.
- 반복 관측되는 배경과 특정 날짜의 짧은 변화를 구분하되, 자료 범위를 넘어 몇 달간 지속한다고 추정하지 않는다. 기간 평균·월평균과 그날의 근거를 구분해 연결한다.
- 전체가 약한데 도움이 되는 개별 근거가 있으면 부분적인 호전과 전체 강세를 구분해 설명한다. 반대 경우도 동일하다.
- 같은 애스펙트의 여러 날짜를 독립된 여러 근거처럼 부풀리지 않는다. 근거의 반복과 종류의 다양성을 구분한다.
- 한 문단에는 새로운 판단 하나만 넣는다. 근거 나열 뒤에 반드시 그 근거가 해당 분야에서 뜻하는 조건을 풀어쓴다. 주의 문장을 모든 문단 끝에 반복하지 않는다.
