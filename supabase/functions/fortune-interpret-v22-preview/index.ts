...
Deno.serve(async(req)=>{
  ...
  let body:any;try{body=await req.json();}catch{return res({ok:false,error:"JSON 요청이 필요해."},400);}
  if(body?.action==="meta"){
  ...
  if(!body?.calculation)return res({ok:false,error:"calculation이 필요해.",gemini_paid_call:false},400);
  const gate=precisionGateFromPayload(body.calculation);
  if(!gate.ok)return res({ok:false,error:"새 정밀도 계약으로 다시 계산해야 AI 해설을 사용할 수 있어.",precision_reason:gate.reason,gemini_paid_call:false},409);
  if(gate.mode==="exact")return proxyV21(req,body);
  let safeCalculation:any;
  try{safeCalculation=sanitizeProvisionalCalculation(body.calculation);const firstAudit=auditProvisionalResidue(safeCalculation);if(!firstAudit.ok)return res({ok:false,error:"잠정 계산에서 시간민감 근거를 안전하게 제거하지 못해 AI 경로를 차단했어.",precision_residue:firstAudit.violations,gemini_paid_call:false},409);}
  catch(e){return res({ok:false,error:`잠정 계산 정리 실패: ${e instanceof Error?e.message:String(e)}`,gemini_paid_call:false},409);}
  if(body?.action==="prompt"){try{return res({ok:true,interpreter_version:VERSION,prompt:buildProvisionalExternalPrompt(body.calculation),gemini_paid_call:false});}catch(e){return res({ok:false,error:`잠정 프롬프트 생성 차단: ${e instanceof Error?e.message:String(e)}`,gemini_paid_call:false},409);}}
  ...
  try{
    ...
    if(!finalAudit.ok)return res({ok:false,error:"잠정 해설 최종 정밀도 감사에서 잔여물이 발견돼 저장을 차단했어.",precision_residue:finalAudit.violations,gemini_paid_call:false},409);
    ...
    if(error||!data?.id)return res({ok:false,error:`잠정 해설 저장 실패: ${error?.message??"unknown"}`,gemini_paid_call:false},500);
  ...
  }catch(e){return res({ok:false,error:`잠정 해설 생성 실패: ${e instanceof Error?e.message:String(e)}`,gemini_paid_call:false},500);}
});
