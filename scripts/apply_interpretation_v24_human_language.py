...
if 'WEEKLY_HEADLINE_ARC_V24' not in source:
    weekly_helper = r'''// WEEKLY_HEADLINE_ARC_V24: a weekly fallback must describe movement across the week, not a daily sentence stretched to seven days.
...
function weeklyEvidenceHeadline(context: FortuneUserSummaryContext): string {
  ...
  const clauses = beats.map(beat=>`${beat}에는 ${beat.topic}에서 ${beat.meaning}이 두드러져 ${beat.scene}`)
...
'''
    marker = 'function buildHeadline(when: string, bestFlow: string[], cautionFlow: string[], consistency: RelationshipConsistency, context: FortuneUserSummaryContext) {'
    if marker not in source:
        raise SystemExit('buildHeadline insertion marker not found')
    source = source.replace(marker, weekly_helper + '\n' + marker, 1)
    branch = """  if (when === '오늘') {
    const scene = dayEvidenceHeadline(context, bestFlow, cautionFlow)
    if (scene) return scene
  }
"""
    branch_new = branch + """  if (when === '이번 주') {
    const arc = weeklyEvidenceHeadline(context)
    if (arc) return arc
  }
"""
    if branch not in source:
        raise SystemExit('daily branch not found for weekly extension')
    source = source.replace(branch, branch_new, 1)
...
weekly_marker = "weekly fallback headline tells a multi-day arc instead of reusing the daily template"
if weekly_marker not in test_source:
    test_source += r'''
