...
current_loading = '''  if (loading) return <section className="ai-interpret-card is-loading"><LoaderCircle className="spin" size={22}/><div><span className="eyebrow">AI INTERPRETATION</span><strong>Gemini가 서버에서 실계산 근거를 해석하는 중…</strong><p>앱을 잠깐 나가도 서버 작업은 계속돼. 돌아오면 완료된 리딩을 자동으로 다시 확인해.</p></div></section>'''
...
if current_loading in s:
    s = s.replace(current_loading, new_loading, 1)
elif new_loading not in s:
    raise AssertionError('AI loading block missing')
...
if old_time in s:
    ...
    raise AssertionError('time topic renderer missing')
