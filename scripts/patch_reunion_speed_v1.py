...
reunion_start = s.index("  const runReunionTiming = async (): Promise<ReunionTimingContext | null> => {")
reunion_end = s.index("\n\n  const runRelationship = async () => {", reunion_start)
new_reunion = r'''  const runReunionTiming = async (): Promise<ReunionTimingContext | null> => {
...
s = s[:reunion_start] + new_reunion + s[reunion_end:]

rel_start = s.index("  const runRelationship = async () => {")
rel_end = s.index("\n\n\n  const runRelationshipAi = async () => {", rel_start)
...
s = s[:rel_start] + new_relationship + s[rel_end:]

old_button = "disabled={relationshipLoading||apiStatus==='offline'}>{relationshipLoading?<LoaderCircle className=\"spin\" size={18}/>:<Sparkles size={18}/>}<span>{relationshipLoading?(selectedTool==='marriage'?'결혼운 계산 중…':relationshipPurpose==='reunion'?'재회운 계산 중…':'궁합 계산 중…')"
new_button = "disabled={relationshipLoading||reunionTimingLoading||apiStatus==='offline'}>{(relationshipLoading||reunionTimingLoading)?<LoaderCircle className=\"spin\" size={18}/>:<Sparkles size={18}/>}<span>{(relationshipLoading||reunionTimingLoading)?(selectedTool==='marriage'?'결혼운 계산 중…':relationshipPurpose==='reunion'?'재회운 계산 중…':'궁합 계산 중…')"
assert old_button in s, "relationship button anchor missing"
s = s.replace(old_button, new_button, 1)

path.write_text(s, encoding="utf-8")
print("patched reunion flow")
