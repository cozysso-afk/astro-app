...
def test_auth_fetch_preserves_current_fortune_transport_and_request_headers():
    auth_source = (ROOT / "web/src/lib/auth.ts").read_text(encoding="utf-8")
    main_source = (ROOT / "web/src/main.tsx").read_text(encoding="utf-8")
    supabase_source = (ROOT / "web/src/lib/supabase.ts").read_text(encoding="utf-8")
    precision_source = (ROOT / "web/src/lib/precisionTransport.ts").read_text(encoding="utf-8")
    assert "new Headers(input instanceof Request ? input.headers : undefined)" in auth_source
    assert "new Headers(init?.headers).forEach" in auth_source
    assert main_source.index("installIntegratedPrecisionFetch()") < main_source.index("ReactDOM.createRoot")
    assert "<AuthGate>" in main_source
    assert "global: { fetch: dynamicFetch }" in supabase_source
    assert "fortune-interpret-v21-preview" in precision_source
    assert "fortune-interpret-v22-preview" in precision_source


def test_private_auth_does_not_reopen_browser_ai_job_access():
    source = (ROOT / "api/private_app.py").read_text(encoding="utf-8")
    lockdown = (
        ROOT / "supabase/migrations/20260911095510_lock_down_ai_interpret_jobs_client_access.sql"
    ).read_text(encoding="utf-8").lower()
    assert "/rest/v1/ai_interpret_jobs" not in source
    assert "revoke all on table public.ai_interpret_jobs from anon, authenticated" in lockdown
    assert "drop policy if exists ai_interpret_jobs_select_own" in lockdown
...
def run_contract() -> None:
    tests = [
        ...
        test_auth_fetch_preserves_current_fortune_transport_and_request_headers,
        test_private_auth_does_not_reopen_browser_ai_job_access,
