...
def run_contract() -> None:
    ...
    assert '@app.get("/v1/relationship/western/jobs/{job_id}/result")' in backend
    ...
    assert "_compact_public_response" in backend
    assert '("daily_trace", "long_term_daily", "saju_boundaries")' in backend
    assert 'job.pop("result", None)' in backend
    assert 'result_ready' in backend
