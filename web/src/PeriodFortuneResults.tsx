...
export function PeriodFortuneResults({
  ...
  return <div className="fortune-experience">
    ...
    {field?.id==='love'&&loveStatus==='single'&&<DatingArchetypePanel key={`${JSON.stringify(datingProfile)}-${result.period.start}-${result.period.end}`} calculation={result} profileGender={profileGender} profile={datingProfile} apiBase={datingApiBase}/>}
