...
export function buildSystemReading(c: IntegratedApiResponse) {
  ...
  const thaiSummary = wheels.length ? `선택한 기간에 적용되는 타크사 연간 배치를 여덟 생활 영역으로 나눠 볼 수 있어. 사람 관계, 생활 리듬, 일, 자원, 실행, 도움, 걸림돌 중 필요한 부분부터 펼쳐봐.` : natalWheel.length ? '출생 때의 타크사 배치를 여덟 생활 영역으로 나눠 볼 수 있어. 각 영역은 생활에서 무엇을 확인하면 되는지 먼저 보여줘.' : '이 기간에 읽을 수 있는 태국점성술 배치가 없어.'
