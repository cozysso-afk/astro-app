...
test('period AI always exposes generation and prompt copy in idle state',()=>{assert.match(ai,/자연어 해설 준비됨/);assert.match(ai,/>해설 생성</);assert.match(ai,/<ExternalPromptCopy onCopy={onCopyPrompt}/);assert.match(readFileSync(new URL('../ExternalPromptCopy.tsx',import.meta.url),'utf8'),/AI 심층해설 프롬프트 복사/)})
