...
export function fortuneFixture(period = 'today') {
  ...
  const calculation = {ok:true,engine:'synthetic',period:{start:'2026-09-12',end,day_count:period==='today'?1:period==='week'?7:period==='month'?19:111,month_segments:1},western:{overall:Object.fromEntries(topics.map(topic=>[topic,stat(scores[topic]??50,period==='today'?0:12)])),relationship_signals:{수신신호:stat(30),발신적합:stat(70)},daily_scores:[{date:'2026-09-12',evidence:[{source_topics:['대인관계'],transit:'Mercury',target:'Jupiter',aspect:'trine',contribution:3,polarity:0.65,text:'W:fixture orb 0.40'},{source_topics:['이직'],transit:'Jupiter',target:'Sun',aspect:'trine',contribution:2,polarity:0.65},{source_topics:['컨디션'],transit:'Mars',target:'Saturn',aspect:'square',contribution:2,polarity:-0.55}]}]},saju:{ok:false},thai:{}}
