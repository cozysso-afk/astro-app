...
test('reunion quality guard preserves narrative depth and keeps technical language out of the main consultation',()=>{
  ...
  assert.match(hierarchyPanel,/연락 가능성 자체는 열려 있어/)
  assert.match(hierarchyPanel,/지금 당장보다 .*에 연락 가능성을 상대적으로 더 살펴볼 수 있어/)
  assert.match(hierarchyPanel,/현재 계산에서는 연락 가능성을 높게 볼 근거가 부족해/)
  assert.match(hierarchyPanel,/실제 메시지 도착을 높은 확률로 확정한다는 뜻은 아니야/)
