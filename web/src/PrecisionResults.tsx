...
type PrecisionEnvelope = IntegratedApiResponse & { precision?: { status?: 'exact'|'provisional'; scoring_mode?: string } }
...
export function PrecisionResults({ result, topTopics, relationshipSignals, topicOrder, actionNotice, archiveStatus, archiveSaving, onCopyPrompt, onCopyResult, onSave }: PrecisionResultsProps) {
  const precision = (result as PrecisionEnvelope).precision
  const provisional = precision?.status === 'provisional'
  const natal = result.western.natal as typeof result.western.natal & { asc: number | null; mc: number | null; house_system?: unknown }
  const angleText = (value: number | null) => typeof value === 'number' && Number.isFinite(value) ? `${value.toFixed(3)}°` : '—'
  return <div className="results-wrap precision-results">
    ...
    <section className="result-card">
      ...
      <div className="precision-kpi-grid">
        <div className="precision-kpi"><span>ASC(상승점)</span><strong>{angleText(natal.asc)}</strong></div>
        <div className="precision-kpi"><span>MC(중천점)</span><strong>{angleText(natal.mc)}</strong></div>
    ...
    <section className="result-card">
      ...
      <div className="precision-table">{topTopics.map(({topic,stat})=>{const best=stat.best_days?.[0];const caution=stat.caution_days?.[0];return <div className="precision-row" key={`precision-${topic}`}><strong>{topic}</strong><span className="precision-score">{stat.average.toFixed(2)}</span><span>{stat.band} · Δ{stat.spread.toFixed(2)}</span><div className="precision-date-stack"><span>↑ {best?`${best.date} ${best.score.toFixed(1)}`:'—'}</span><span>↓ {caution?`${caution.date} ${caution.score.toFixed(1)}`:'—'}</span></div></div>})}</div>
    </section>

    {relationshipSignals.length>0 && <section className="result-card"><div className="result-card-title"><span>RELATIONSHIP SIGNALS</span><strong>관계 관련 기간 신호</strong></div><div className="precision-table">{relationshipSignals.map(([topic,stat])=><div className="precision-row" key={`relationship-signal-${topic}`}><strong>{topic}</strong><span className="precision-score">{stat.average.toFixed(2)}</span><span>{stat.band}</span><span>변동폭 {stat.spread.toFixed(2)}</span></div>)}</div><p className="result-note">이 값도 연락·재회·결혼의 사건 확률이 아니라 상대적 활성도야.</p></section>}

    {result.western.months.length>0 && <section className="result-card"><div className="result-card-title"><span>MONTH RAW</span><strong>월별 전체 지표</strong></div>{result.western.months.map((month)=><details className="precision-details" key={`precision-month-${month.calendar_month}`}><summary>{month.calendar_month} · {month.start}~{month.end}</summary><div className="precision-details-body"><div className="precision-table">{topicOrder.map((topic)=>{const stat=month.topics[topic];return stat?<div className="precision-row" key={`${month.calendar_month}-${topic}`}><strong>{topic}</strong><span className="precision-score">{stat.average.toFixed(2)}</span><span>{stat.band}</span><span>Δ {stat.spread.toFixed(2)}</span></div>:null})}</div></div></details>)}</section>}

    <section className="result-card">
      <div className="result-card-title"><span>SAJU RAW</span><strong>사주 계산 원자료{provisional?' · 잠정 참고':''}</strong></div>
      {provisional && <p className="result-note">입력 시각 기반 참고 원자료야. 이번 잠정 모드의 AI 근거에는 사용하지 않아.</p>}
      {result.saju.ok && result.saju.pillars ? <>
        <div className="pillar-grid"><div><span>년주</span><strong>{result.saju.pillars.year}</strong></div><div><span>월주</span><strong>{result.saju.pillars.month}</strong></div><div><span>일주</span><strong>{result.saju.pillars.day}</strong></div><div><span>시주</span><strong>{result.saju.pillars.hour}</strong></div></div>
    ...
    <section className="result-card">
      <div className="result-card-title"><span>THAI STATUS</span><strong>태국점성술 계산 상태{provisional?' · 잠정 참고':''}</strong></div>
      {provisional && <p className="result-note">입력 시각 기반 참고 원자료야. 이번 잠정 모드의 AI 근거에는 사용하지 않아.</p>}
      ...
      <div className="tight-row"><span>설명 범위</span><b>검증된 위치·하우스 맥락만</b></div><div className="tight-row"><span>예측 제한</span><b>사건·시기·확률·점수 미제공</b></div>
    ...
    <section className="result-card"><div className="result-card-title"><span>RAW JSON</span><strong>원본 계산 응답</strong></div><details className="precision-details"><summary>원본 JSON(제이슨·데이터 형식) 전체 펼치기</summary><div className="precision-details-body"><pre className="precision-json">{JSON.stringify(result,null,2)}</pre></div></details></section>
  </div>
}
