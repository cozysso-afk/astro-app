...
export function SettingsView({
  ...
  const apiStatusLabel = apiStatus === 'online' ? '연결됨' : apiStatus === 'warming' ? '확인 중' : '대기 중'
  const apiStatusDetail = apiStatus === 'online' ? '계산 기능 정상' : apiStatus === 'warming' ? '연결 상태 확인 중' : '잠시 후 다시 확인해줘'
  const aiStatusLabel = aiConfigured === true ? '연결됨' : aiConfigured === false ? '미연결' : '확인 중'
  const aiStatusDetail = aiConfigured === true ? 'Gemini 해설 사용 가능' : aiConfigured === false ? 'API 설정이 필요해' : '연결 상태 확인 중'
  const aiModelLabel = aiModel === 'gemini-3.6-flash' ? 'Gemini 3.6 Flash' : 'Gemini 3.7 Flash'
  const pushStatusLabel = pushState?.status === 'ready'
    ? '켜짐'
    : pushState?.status === 'needs_install'
      ? '홈화면 설치 필요'
  ...
  return <section className="form-card settings-view">
    ...
    <div className="ai-settings-card">
      ...
      <div className={`ai-api-state ${aiConfigured===true?'online':aiConfigured===false?'offline':'checking'}`}><Sparkles size={16}/><span><strong>AI 해설 연결</strong><small>{aiStatusDetail}</small></span></div>
    </div>

    <div className="subsection-title">알림</div>
    <div className="push-settings-card">
      <div className={`push-state-row ${pushState?.status || 'checking'}`}><span className="push-state-icon">🔔</span><span><strong>운세 푸시 알림</strong><small>{pushState?.message || '알림 구독 상태 확인 중'}</small></span></div>
      ...
      {pushState?.status==='error' && <p>알림 연결을 확인하지 못했어. 잠시 후 다시 시도해줘.</p>}
    </div>

    <div className="subsection-title">앱 상태</div>
    <div className="settings-status-grid">
      <div><span>계산 서버</span><strong>{apiStatusLabel}</strong><small>{apiStatusDetail}</small></div>
      <div><span>AI 해설</span><strong>{aiStatusLabel}</strong><small>{aiModelLabel}</small></div>
      <div><span>알림</span><strong>{pushStatusLabel}</strong><small>{pushState?.message || '설정 탭에서 확인'}</small></div>
