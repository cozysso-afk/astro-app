...
export function RelationshipInterpretationPanel({ sajuContext, aspects, partnerExact, ai, aiLoading, aiError, onAi, analysisMode, timeSensitivePoints, formatAspect, timing, technicalDetails }: {
  ...
  const directionLead = (() => {
    if (!reunion) return ''
    const incoming = timing?.incoming?.average
    const outgoing = timing?.outgoing?.average
    if (typeof incoming !== 'number' || typeof outgoing !== 'number' || !Number.isFinite(incoming) || !Number.isFinite(outgoing)) return '누가 먼저 움직일지를 비교할 방향별 계산값이 충분하지 않아.'
    const gap = incoming - outgoing
    if (Math.abs(gap) < 4) return '상대 → 나와 나 → 상대 흐름의 차이가 크지 않아. 먼저 움직이는 쪽을 한쪽으로 단정하기 어려워.'
    return gap > 0 ? '선택 기간 계산에서는 상대 → 나 방향이 더 강해. 다만 실제 연락이 온다는 사건 확률은 아니야.' : '선택 기간 계산에서는 나 → 상대 방향이 더 강해. 먼저 연락한다면 상대 반응을 보고 다음 행동을 정하는 편이 맞아.'
  })()
  const directionRows = [{kind:'incoming' as const,label:'상대 → 나',...view.incoming},{kind:'outgoing' as const,label:'나 → 상대',...view.outgoing},{kind:'reconnection' as const,label:'과거 인연 재접점',...view.reconnection}]
  ...
  return <section className="relationship-experience reading-experience" data-mode={analysisMode}>
    ...
    {reunion&&<section className="reading-section reunion-direct-answer"><h3>재회 핵심 · 연락 방향과 재접점</h3><p className="reunion-direction-lead">{directionLead}</p><ReadingDirections rows={directionRows}/><div className="reunion-window-block"><h4>주목 시기</h4>{view.windows.length ? <ReadingTimeline events={view.windows.slice(0,6).map(w=>({date:w.date,kind:w.kind,label:w.label,status:w.status,detail:w.detail}))}/> : <p>선택 기간 안에서 다른 날과 구별할 만큼 뚜렷한 시기 신호는 없어.</p>}</div><p className="reading-muted reunion-score-note">강함·보통·약함은 선택 기간 안의 상대적 활성도야. 실제 연락 여부나 재회 확률 %를 뜻하지 않아.</p></section>}
    {ai?.ok && ai.data ? reunion ? <section className="reading-section relationship-natural-reading reunion-natural-reading"><h3>{ai.data.headline}</h3><p className="reading-conclusion">{ai.data.reunion_reading?.bottom_line || ai.data.overview}</p><div className="reunion-ai-direction-grid"><article><h4>상대 → 나</h4><p>{ai.data.reunion_reading?.incoming_contact || '추가 해설이 없어.'}</p></article><article><h4>나 → 상대</h4><p>{ai.data.reunion_reading?.outgoing_contact || '추가 해설이 없어.'}</p></article></div><section className="reunion-ai-block"><h4>재접점 · 시기</h4><p>{ai.data.reunion_reading?.reconnection_windows || ai.data.timing}</p>{ai.data.reunion_reading?.low_windows&&<p className="reading-muted">힘이 덜 실리는 구간 · {ai.data.reunion_reading.low_windows}</p>}</section><section className="reunion-ai-block"><h4>연락 이후 관계 회복을 볼 기준</h4><p>{ai.data.reunion_reading?.relationship_filter || ai.data.long_term}</p></section>{ai.data.reunion_reading?.precision_note&&<ReadingExplanation kind="caution">{ai.data.reunion_reading.precision_note}</ReadingExplanation>}</section> : <section className="reading-section relationship-natural-reading"><h3>{ai.data.headline}</h3><p className="reading-conclusion">{analysisMode.startsWith('marriage_') ? ai.data.marriage_reading?.bottom_line || ai.data.overview : ai.data.overview}</p>{analysisMode.startsWith('marriage_') ? <><ReadingExplanation kind="reason">{[ai.data.marriage_reading?.bond,ai.data.marriage_reading?.emotional_home].filter(Boolean).join(' ')}</ReadingExplanation><ReadingExplanation kind="practice">{[ai.data.marriage_reading?.daily_life,ai.data.marriage_reading?.intimacy_resources,ai.data.marriage_reading?.conflict_repair].filter(Boolean).join(' ')}</ReadingExplanation><ReadingExplanation kind="timing">{[ai.data.marriage_reading?.commitment_or_current_cycle,ai.data.marriage_reading?.timing].filter(Boolean).join(' ')}</ReadingExplanation><ReadingExplanation kind="caution">{[ai.data.marriage_reading?.caution,ai.data.marriage_reading?.precision_note].filter(Boolean).join(' ')}</ReadingExplanation></> : <><ReadingExplanation kind="reason">{[ai.data.chemistry,ai.data.emotional_dynamic,ai.data.communication].filter(Boolean).join(' ')}</ReadingExplanation><ReadingExplanation kind="practice">{[ai.data.long_term,...(ai.data.practical_advice??[])].filter(Boolean).join(' ')}</ReadingExplanation><ReadingExplanation kind="timing">{ai.data.timing}</ReadingExplanation><ReadingExplanation kind="caution">{[ai.data.conflict_pattern,ai.data.power_boundaries].filter(Boolean).join(' ')}</ReadingExplanation></>}</section> : <section className="reading-section relationship-natural-reading is-ready"><h3>Gemini 자연어 해설</h3><p>{reunion ? '위 계산값을 바탕으로 상대 → 나, 나 → 상대, 재접점 시기와 연락 이후 관계 회복 조건을 근거별로 풀어줘.' : '계산은 끝났어. 저장된 자연어 해설이 없으면 한 번 생성해. 생성한 해설은 다시 비용이 들지 않도록 저장해둘게.'}</p><p className="ai-preflight-cost">{relationshipAiCostPreview(analysisMode)}</p><button type="button" onClick={onAi} disabled={aiLoading}><Sparkles size={16}/>{aiLoading ? '관계 흐름을 정리하고 있어…' : '자연어 해설 생성'}</button>{aiError && <p className="reading-alert" role="status"><AlertTriangle size={16}/>{aiError}</p>}</section>}
    ...
    {reunion ? <>
      <section className="reading-section"><h3 className="reading-heading-with-badges"><span>{view.stabilityTitle}</span><ReadingBadge kind="rebuilding"/><span className="reading-state">{view.sustainability}</span></h3><p>{view.sustainabilityText}</p></section>
