...
export function RelationshipInterpretationPanel({ aspects, partnerExact, ai, aiLoading, aiError, onAi, analysisMode, timeSensitivePoints, formatAspect }: { aspects: Aspect[]; partnerExact: boolean; ai: RelationshipAiResponse | null; aiLoading: boolean; aiError: string; onAi: () => void; analysisMode: RelationshipAnalysisMode; timeSensitivePoints: ReadonlySet<string>; formatAspect: (aspect: Aspect) => string }) {
  const interpretableAspects = partnerExact ? aspects : aspects.filter((a)=>!timeSensitivePoints.has(a.a) && !timeSensitivePoints.has(a.b))
  ...
  const headline = topAspect ? `${formatAspect(topAspect)}이 가장 강하게 걸리는 관계` : '확정 가능한 핵심 접점이 적은 관계 구조'
  const overview = tight.length ? tight.slice(0,3).map((aspect,index)=>`${index+1}순위 ${formatAspect(aspect)}(오브 ${aspect.orb.toFixed(2)}°): ${relationshipAspectMeaning(aspect)}`).join(' ') : '현재 입력으로 확정 가능한 주요 접점이 적어서 관계 전체를 강하게 단정하기 어려워.'
  ...
  const communicationText = communicationTight.length ? communicationTight.map((aspect)=>`${formatAspect(aspect)}(오브 ${aspect.orb.toFixed(2)}°) — ${relationshipAspectMeaning(aspect)}`).join(' ') : 'Mercury(수성) 관련 확정 접점이 상위권에 적어서 소통 패턴은 현재 차트만으로 강하게 말하지 않을게.'
  const chemistryText = chemistryTight.length ? chemistryTight.map((aspect)=>`${formatAspect(aspect)}(오브 ${aspect.orb.toFixed(2)}°) — ${relationshipAspectMeaning(aspect)}`).join(' ') : 'Venus(금성)·Mars(화성)·Pluto(명왕성) 관련 확정 접점이 상위권에 적어서 끌림 하나로 관계를 설명하진 않을게.'
  const stabilityText = structureTight.length ? structureTight.map((aspect)=>`${formatAspect(aspect)}(오브 ${aspect.orb.toFixed(2)}°) — ${relationshipAspectMeaning(aspect)}`).join(' ') : 'Saturn(토성)·Jupiter(목성)·교점 관련 확정 접점이 상위권에 적어서 장기 지속성은 현재 계산만으로 강하게 단정하기 어려워.'
  ...
  return <>
    <section className="relationship-reading-card">
      ...
      <div className="relationship-key-aspects"><strong>가장 강한 접점</strong>{tight.slice(0,3).map((aspect,index)=><div key={`${aspect.a}-${aspect.aspect}-${aspect.b}-${index}`}><b>{formatAspect(aspect)} · 오브 {aspect.orb.toFixed(2)}°</b><p>{relationshipAspectMeaning(aspect)}</p></div>)}</div>
    </section>
