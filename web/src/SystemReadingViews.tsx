...
import { buildSystemReading, BHUMI_LENSES, tenGodLens, SAJU_LIFE_KEYS, THAI_LIFE_KEYS, compactSystemPrompt, type LifeTopic, type SystemId, type Lens } from './lib/systemReading'
import { ReadingExplanation } from './ReadingExplanation'

const SYSTEMS = [{id:'integrated',label:'통합',Icon:Layers3},{id:'western',label:'서양점성술',Icon:Orbit},{id:'saju',label:'사주',Icon:Columns3},{id:'thai',label:'Thai',Icon:Sparkles}] as const
...
export function SystemReadingViews({calculation:c,children,field,initialSystem='integrated'}:{calculation:IntegratedApiResponse;children:ReactNode;field?:FortuneField;initialSystem?:SystemId}) {
  ...
  const overview = <section className="system-overview"><h3>세 체계 한눈에</h3><div className="system-overview-grid">
        ...
        {view.thai&&(view.natalWheel.length||view.wheels.length)>0 ? <article className="system-thai"><strong><Sparkles size={16}/>Thai</strong><p>{topic==='전체'?view.thaiSummary:selectedBhumi.length?selectedBhumi.map(r=>`${BHUMI_LENSES[r.bhumi_key].title}에 ${r.planet.label}`).join(', '):'이 분야에 대응하는 Bhumi 근거가 없어.'}</p><small>생활 영역의 맥락이며 길흉 점수가 아니야.</small></article>:<p className="system-unavailable">Thai · 현재 정밀도 또는 사용 가능한 배치가 부족해.</p>}
      </div><details className="system-synthesis"><summary>세 체계를 같이 보면 · {view.state}</summary><p>서양점성술의 분야 강약과 사주의 운 구간, Thai의 생활 영역 배치는 서로 다른 질문에 답해. 숫자를 더하거나 같은 결론으로 맞추지 않고, 선택한 분야에서 실제로 겹치는 맥락이 있는지 비교해.</p>{selectedLenses.map(l=><p key={l.key}>{l.title} 맥락에서는 {l.action}</p>)}</details></section>
  return <section className={`system-reading system-${system}`}>
    ...
    {system==='integrated' ? <>
    ...
    </> : !view.allowed ? <p className="system-unavailable">출생시간 검증 정책에 따라 이 계산에서는 {system==='saju'?'사주':'Thai'} 해석이 제외돼 있어. 탭 전환으로 정밀도 제한을 바꾸지 않아.</p> : system==='saju' ? <>
      ...
      <header className="system-hero"><span>Thai · {period}</span><h3>{view.thaiSummary}</h3><p>8 Bhumi는 생활의 어느 영역을 살펴보는지 보여줘. 모든 영역에 행성이 배치되는 구조이므로 배치가 있다는 이유만으로 그 분야가 강하다고 판정하지 않아.</p></header>
      <div className="system-topic-selector" role="group" aria-label="Thai 운 구간 선택">{!!view.natalWheel.length&&<button type="button" aria-pressed={wheelIndex===-1} onClick={()=>setWheelIndex(-1)}>출생 Mahathaksa</button>}{view.wheels.map((w,i)=><button type="button" key={w.start} aria-pressed={wheelIndex===i} onClick={()=>setWheelIndex(i)}>{w.start}–{w.end}</button>)}</div>
      {view.wheels.map((w,i)=><details className="system-segment" key={w.start} open={wheelIndex===i}><summary>Taksajorn · {w.start}–{w.end}<small>연간 Boriwan · {w.annual_boriwan.label}</small></summary><p>진행 나이 {w.age_in_progress}의 실제 배치야. {w.landed_center?'중앙에 도달해 엔진의 목성 대체 규칙을 적용했어.':'이 구간의 연간 Boriwan을 시작점으로 배치가 기록돼 있어.'} 전후 구간이 바뀌더라도 사건의 발생 시각을 뜻하지 않아.</p></details>)}
      <section><h3>8 Bhumi · 생활 영역 지도</h3><div className="bhumi-grid">{selectedBhumi.map(r=><details key={r.bhumi_key}><summary><small>{r.bhumi_key}</small><strong>{r.planet.label}</strong><span>{BHUMI_LENSES[r.bhumi_key].title}</span></summary><p>{BHUMI_LENSES[r.bhumi_key].meaning}</p></details>)}</div></section>
      <section><h3>생활 분야로 읽기</h3>{selectedBhumi.map((r,i)=><details className="system-segment" key={r.bhumi_key} open={i<2}><summary>{BHUMI_LENSES[r.bhumi_key].title}</summary><LensCard lens={BHUMI_LENSES[r.bhumi_key]} evidence={`${r.bhumi_label} · ${r.planet.label}. 현재 선택한 wheel의 실제 배치이며 서양점성의 행성 길흉을 가져오지 않았어.`}/></details>)}</section>
      <details className="system-raw"><summary>Thai 계산 범위 자세히 보기</summary><p>출생 요일 · {view.thai?.thai_day} · 출생 행성 {view.thai?.birth_planet?.label}</p><p>{view.thai?.predictive_status}</p><p>{view.thai?.consensus_policy}</p><p>미계산: {view.thai?.not_calculated?.join(', ')}</p></details>
