...
import { lensForTopic, thaiPlacementComparison, thaiPlanetLabel, thaiLifeSummary, buildSystemReading, BHUMI_LENSES, tenGodLens, SAJU_LIFE_KEYS, THAI_LIFE_KEYS, compactSystemPrompt, type LifeTopic, type SystemId, type Lens } from './lib/systemReading'
...
export function SystemReadingViews({calculation:c,children,field,loveStatus,initialSystem='integrated'}:{calculation:IntegratedApiResponse;children:ReactNode;field?:FortuneField;loveStatus?:LoveStatus;initialSystem?:SystemId}) {
  ...
  return <section className={`system-reading system-${system}`}>
    ...
    {system==='integrated' ? <>
      ...
      <header className="system-hero"><span>태국점성술 · {period}</span><h3>{view.thaiSummary}</h3><p>주변 사람, 생활 리듬, 자원과 실행을 서로 다른 영역으로 나눠 읽는 방식이야. 아래에서 출생 배치와 선택 기간의 연간 배치를 비교할 수 있어.</p></header>
      ...
      <section className="thai-life-map"><h3>여덟 생활 영역</h3><p className="thai-map-intro">궁금한 영역을 펼치면 배치와 생활에서 읽는 방법이 나와.</p><div className="bhumi-grid">{selectedBhumi.map(r=><details data-bhumi={r.bhumi_key} key={r.bhumi_key}><summary><strong>{BHUMI_LENSES[r.bhumi_key].title}</strong><span className="thai-placement">{thaiPlanetLabel(r.planet.label)} 배치 <span aria-hidden="true">＋</span></span></summary><div className="thai-lens-reading"><p>{thaiPlacementComparison(r,view.natalWheel,wheelIndex<0||!view.wheels.length)}</p><p>{BHUMI_LENSES[r.bhumi_key].meaning}</p><ReadingExplanation kind="practice">{BHUMI_LENSES[r.bhumi_key].action}</ReadingExplanation><small>선택한 배치에서 이 영역을 담당하는 행성은 {thaiPlanetLabel(r.planet.label)}이야.</small></div></details>)}</div></section>
      <details className="system-raw thai-reading-limit"><summary>이 해설을 읽는 범위</summary><p>각 영역의 의미를 생활에 적용한 해설이야. 행성이 배치됐다는 이유만으로 좋은 운·나쁜 운을 정하거나 사건의 성립, 상대 마음, 건강 상태를 판단하지 않아.</p></details>
