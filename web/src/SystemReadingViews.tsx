...
export function SystemReadingViews({calculation:c,children,field,loveStatus,initialSystem='integrated'}:{calculation:IntegratedApiResponse;children:ReactNode;field?:FortuneField;loveStatus?:LoveStatus;initialSystem?:SystemId}) {
  ...
  const overview = <section className="system-overview"><h3>세 체계 한눈에</h3><div className="system-overview-grid">
        ...
        {view.thai&&(view.natalWheel.length||view.wheels.length)>0 ? <article className="system-thai"><strong><Sparkles size={16}/>태국점성술</strong><p>{topic==='전체'?view.thaiSummary:selectedBhumi.length?thaiLifeSummary(selectedBhumi.map(r=>r.bhumi_key)):'이 분야와 연결된 생활 영역 자료가 없어.'}</p><small>생활 영역을 읽는 기준표이며 길흉 점수는 아니야.</small></article>:<p className="system-unavailable">태국점성술 · 현재 정밀도 또는 사용 가능한 배치가 부족해.</p>}
  ...
  return <section className={`system-reading system-${system}`}>
    ...
    {system==='integrated' ? <>
      ...
      <header className="system-hero"><span>태국점성술 · {period}</span><h3>{view.thaiSummary}</h3><p>먼저 생활에서 무엇을 확인하면 되는지 읽고, 행성 배치는 아래쪽 계산 근거에서 확인할 수 있어.</p></header>
      ...
      {view.wheels.map((w,i)=><details className="system-segment" key={w.start} open={wheelIndex===i}><summary>{thaiPeriodLabel(w.start,w.end)}<small>계산 기준 · 주변 사람 영역 {thaiPlanetLabel(w.annual_boriwan.label)}</small></summary><p>진행 나이 {w.age_in_progress}에 적용되는 타크사 연간 기준표야. 아래에서는 계산 과정보다 사람 관계, 생활 리듬, 일, 자원, 실행, 도움, 걸림돌처럼 실제 생활에서 확인할 내용을 먼저 보여줘.</p>{w.landed_center&&<small>계산상 중앙에 도달해 엔진의 목성 대체 규칙이 적용됐어.</small>}</details>)}
      <section className="thai-life-map"><h3>여덟 생활 영역</h3><p className="thai-map-intro">궁금한 영역을 펼치면 ‘무슨 뜻인지 → 현실에서 볼 것 → 계산 근거’ 순서로 볼 수 있어.</p><div className="bhumi-grid">{selectedBhumi.map(r=><details data-bhumi={r.bhumi_key} key={r.bhumi_key}><summary><strong>{BHUMI_LENSES[r.bhumi_key].title}</strong><span className="thai-placement">계산값 · {thaiPlanetLabel(r.planet.label)}</span></summary><div className="thai-lens-reading"><p>{BHUMI_LENSES[r.bhumi_key].meaning}</p><ReadingExplanation kind="practice">{BHUMI_LENSES[r.bhumi_key].action}</ReadingExplanation><ReadingExplanation kind="reason">{thaiPlacementComparison(r,view.natalWheel,wheelIndex<0||!view.wheels.length)}</ReadingExplanation></div></details>)}</div></section>
