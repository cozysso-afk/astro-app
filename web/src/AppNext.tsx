...
import { PrecisionResults } from './PrecisionResults'
...
const FORTUNE_AI_START_ERROR_FALLBACK = 'AI 해설 서버에서 오류가 발생했어. 설정의 AI 해설 연결 상태를 확인해줘.'
...
async function fortuneAiStartErrorMessage(error: unknown, invokeData: unknown) {
  ...
  const isHttpError = Boolean(
    context != null
    || (error instanceof Error && (error.name === 'FunctionsHttpError' || error.message.includes('non-2xx')))
  )
  if (!isHttpError) {
      ...
      <main className="page-content">
        ...
        {mainView === 'home' && <>
          ...
          {selectedTool === 'precision' && <section className="tool-panel precision-panel">
            ...
            {integratedMatchesSelection && integratedResult && <PrecisionResults
              result={integratedResult}
              topTopics={topIntegratedTopics}
              relationshipSignals={precisionRelationshipSignals}
