...
export function FortuneFlowCards({ title, items, caution = false }: { title: string; items: FortuneFlowCard[]; caution?: boolean }) {
  const reference = title === '참고할 흐름'
  const card = (item: FortuneFlowCard) => {
    const Icon = icons[item.topic as keyof typeof icons] ?? TrendingUp
    const referenceTint = reference ? ({'--opal-tint':'#e6ddf6'} as CSSProperties) : undefined
    return <article className={`flow-tile${reference ? ' is-reference-flow-tile' : ''}`} style={referenceTint} key={item.topic}>
