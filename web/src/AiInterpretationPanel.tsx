...
export function AiInterpretationPanel({ result, loading, error, onRetry, onCopyPrompt, onCancel, canCancel, topics }: {
  ...
  return <section className="ai-interpret-card">
    <div className="ai-interpret-head"><span className="ai-orb"><Sparkles size={19}/></span><div><span className="eyebrow">Gemini(제미나이) 통합 해설</span><h3>{visibleAiText(data.headline) || '통합 계산 해설'}</h3><small>실계산 결과 · 월별 변화 · 핵심 날짜를 함께 읽는 해설</small></div></div>
    ...
    {hasKeyWindows && <section className="ai-quick-dates"><div className="ai-section-heading"><span>가장 먼저 볼 날짜</span><strong>먼저 볼 시기</strong></div><div className="ai-quick-date-list">{data.key_windows!.slice(0,3).map((item,index)=><button type="button" className={`ai-quick-date ${signalClass(item.signal)}`} key={`quick-${item.start}-${index}`} onClick={()=>inspectAnnualScore(item.start,item.topics?.[0])}><b>{periodLabel(item.start,item.end)}</b><span><strong>{visibleAiText(item.label)}</strong>{item.topics?.length?<small>{item.topics.slice(0,3).join(' · ')}</small>:null}</span><em>{item.signal}</em></button>)}</div></section>}

    {hasDecisions && <section className="ai-decision-section"><div className="ai-section-heading"><span>먼저 이것부터</span><strong>이 기간에 실제로 할 일</strong></div><div className="ai-decision-list">{data.decisions!.map((item,index)=><article key={`${index}-${item.action}`}><span className="ai-decision-index">{index+1}</span><div><strong>{visibleAiText(item.action)}</strong>{item.timing&&<b>{item.timing}</b>}{item.watch&&<div className="ai-decision-condition"><b>확인</b><span>{item.watch}</span></div>}<details className="ai-decision-more"><summary>근거 · 주의 보기</summary>{item.reason&&<p>{visibleAiText(item.reason)}</p>}{item.avoid&&<div className="ai-decision-condition is-avoid"><b>피할 것</b><span>{item.avoid}</span></div>}<small>계산 근거 {item.evidence_refs?.length ?? 0}개 연결</small></details></div></article>)}</div></section>}

    {hasKeyWindows && <section className="ai-key-window-section"><div className="ai-section-heading"><span>중요 날짜 · 시기</span><strong>눈여겨볼 핵심 구간</strong></div><div className="ai-key-window-list">{data.key_windows!.map((item,index)=><article className={`ai-key-window ${signalClass(item.signal)}`} key={`${item.start}-${item.end}-${index}`}><div className="ai-key-window-top"><div><span className="ai-window-date">{periodLabel(item.start,item.end)}</span><strong>{visibleAiText(item.label)}</strong></div><b>{item.signal}</b></div>{item.topics?.length?<div className="ai-window-topics">{item.topics.map((topic)=><span key={topic}>{topic}</span>)}</div>:null}<p>{visibleAiText(item.summary)}</p>{!hasDecisions&&item.action&&<div className="ai-window-action"><strong>이때</strong><span>{item.action}</span></div>}{item.avoid&&<div className="ai-window-avoid"><strong>피할 것</strong><span>{item.avoid}</span></div>}<div className="ai-window-footer"><small>계산 근거 {item.evidence_refs?.length ?? 0}개 검증</small><button type="button" onClick={()=>inspectAnnualScore(item.start,item.topics?.[0])}>날짜 점수에서 확인</button></div></article>)}</div></section>}

    {showRelationshipFocus && relationshipReading ? <section className="ai-relationship-section"><div className="ai-section-heading"><span>관계 · 연락 · 재회</span><strong>세 방향을 따로 보면</strong></div><div className="ai-highlight ai-relationship-highlight"><strong>관계 방향 요약</strong><span>{visibleAiText(relationshipReading.flow)}</span></div>{data.contact_flow && (data.contact_flow.incoming || data.contact_flow.outgoing || data.contact_flow.reconnection) ? <div className="ai-direction-grid ai-relationship-direction"><details className="ai-direction-item"><summary><strong>상대 → 나</strong><span>설명 보기</span></summary><p>{visibleAiText(data.contact_flow.incoming || '뚜렷한 수신 근거가 없어.')}</p></details><details className="ai-direction-item"><summary><strong>나 → 상대</strong><span>설명 보기</span></summary><p>{visibleAiText(data.contact_flow.outgoing || '뚜렷한 발신 적합 근거가 없어.')}</p></details><details className="ai-direction-item"><summary><strong>과거 인연 재접점</strong><span>설명 보기</span></summary><p>{visibleAiText(data.contact_flow.reconnection || '재접점 근거가 약해.')}</p></details></div> : null}<div className="ai-relationship-timing"><b>주목 시기</b><span>{visibleAiText(relationshipReading.focus_timing)}</span></div><details className="ai-relationship-more"><summary>판단 기준 · 주의 보기</summary><p><b>세 축 기준</b> {visibleAiText(relationshipReading.context)}</p><p><b>현실에서 확인</b> {visibleAiText(relationshipReading.watch)}</p><p><b>과대해석 주의</b> {visibleAiText(relationshipReading.avoid)}</p></details></section> : null}
