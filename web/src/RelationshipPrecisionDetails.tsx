...
export function RelationshipPrecisionDetails({ result, partnerTimeExact, aspects, formatLimit }: RelationshipPrecisionDetailsProps) {
  ...
  return <>
    {partnerTimeExact && houseOverlays?.available && <section className="result-card"><div className="result-card-title"><span>HOUSE OVERLAY</span><strong>홀사인 + 플라시두스 관계 하우스</strong></div><p className="result-note">한 체계로 덮어쓰지 않고 둘 다 보여줘. 숫자가 다르면 서로 다른 해석층이고, 같으면 중첩 근거로 봐.</p><div className="month-list">{[
